...
Users can specify learners for each nuisance function through nuisance-specific arguments `m_learners`, `g_learners`, `r_learners` and `po_learners`, as well as set default learners through the `default_learners` argument. To estimate a particular nuisance function through Super Learner, users can specify Super Learner libraries using the same syntax one passes directly into `SuperLearner`. To see the SuperLearner libraries available, users can rub `get_sl_libraries()`:
...
To estimate all nuisance functions with the highly-adaptive LASSO, users can specify the `default_learners` argument as `SL.hal9001`.
