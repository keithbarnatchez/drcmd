...
To illustrate the breadth of missingness patterns `drcmd` can accommodate, we outline a non-exhaustive set of example problems that can be directly addressed by `drcmd`. Throughout, assume that the causal inference assumptions of consistency, positivity and unconfoundedness hold across all data structures provided below.
...
$$
(Y_i, A_i, V_i, R_i U_i, R_i), \quad i = 1,\ldots,n
$$
...
While the methods underpinning `drcmd` have most commonly leveraged in two-phase settings like the one above, their use is applicable in broader missing data problems. Suppose the observed data structure is
$$
